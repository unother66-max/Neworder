...
### 매일 새벽 2시 30분: 신규 후보 수집

목표:

- 신규 후보 블로그 최대 100개 수집
- seed keyword별 검색 결과에서 `blogId` 추출
- `BlogDiscoveryQueue`에 upsert

route:

- `POST /api/cron/blog-discovery`

Vercel Cron:

- KST 02:30 = UTC 전날 17:30
- schedule: `30 17 * * *`
- 자동 실행 기본값: `keywordLimit=5`, `resultsPerKeyword=10`, `maxSaveCount=100`

### 매일 새벽 3시 30분: 후보 분석 전환

목표:

- `BlogDiscoveryQueue`의 pending 후보 중 최대 3개만 분석
- 실패 시 전체 크론 실패로 만들지 않고 큐 상태만 갱신
- 분석 성공 시 `BlogProfile`, `BlogMetricSnapshot` 업데이트

route:

- `POST /api/cron/blog-discovery-analyze`

Vercel Cron:

- KST 03:30 = UTC 전날 18:30
- schedule: `30 18 * * *`
- 자동 실행 기본값: `limit=3`, 최대 `10`
...
- `POST /api/cron/blog-rank-snapshot`

Vercel Cron:

- KST 월요일 04:00 = UTC 일요일 19:00
- schedule: `0 19 * * 0`
...
- 중복 blogId 제거
- 이미 `BlogProfile`에 있는 분석 완료 블로그 제외
...
- 자동 실행 기본값: keywordLimit 5개
- 자동 실행 기본값: resultsPerKeyword 10개
- 자동 실행 기본값: maxSaveCount 100개
- route 안전 상한: keywordLimit 최대 20개
- route 안전 상한: resultsPerKeyword 최대 30개
- route 안전 상한: maxSaveCount 최대 500개
...
- Vercel Cron schedule: `30 17 * * *`

### `/api/cron/blog-discovery-analyze`

역할:

- `BlogDiscoveryQueue`의 pending 후보를 소량 분석
- `BlogProfile`, `BlogMetricSnapshot` 생성
- 성공 시 후보 상태를 `analyzed`로 변경
- 실패 시 후보 상태를 `failed`로 변경하고 errorMessage 저장

초기 안전 제한:

- 자동 실행 기본값: limit 3개
- route 안전 상한: limit 최대 10개
- 후보 사이 750ms delay

자동 실행:

- Vercel Cron schedule: `30 18 * * *`
...
- 이 단계에서는 후보만 저장하고 `BlogProfile`, `BlogMetricSnapshot`은 생성하지 않음
