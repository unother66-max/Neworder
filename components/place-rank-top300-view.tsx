...
import { readPlaceRankAnalysisKeyword } from "@/lib/place-rank-analysis-query";
...
const SAMPLE_PLACE_RANK_ROWS = [
  ...
  (row, index): PlaceRankRow => {
    const saveCountValue = Number(row.saveCount.replace(/,/g, ""));
    return {
      rank: index + 1,
      ...row,
      saveCountValue,
...
export default function PlaceRankTop300View({
  mode = "page",
  ...
  const previousRanks = useMemo(
    () =>
      selectedSnapshot
        ? buildPreviousTop300RankMap(
            selectedSnapshot.rankedPlaceIds,
            selectedSnapshot.rankOverrides
          )
        : null,
    [selectedSnapshot]
  );
  const previousMetrics = useMemo(
    () =>
      selectedSnapshot
        ? buildPreviousTop300MetricMap(selectedSnapshot.metrics)
        : null,
    [selectedSnapshot]
  );
  ...
  useEffect(() => {
    if (autoSearchStartedRef.current || status === "loading") {
      return;
    }
  ...
  }, [modalKeyword, mode, runSearch, status]);

  const handleSearch = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (status === "loading") return;
    if (guardAction(event)) return;
    if (loading || searchInFlightRef.current) return;
  ...
  const content = (
    ...
    >
          ...
          {mode === "modal" && loading ? (
            <div
              role="status"
              className="flex min-h-[240px] flex-col items-center justify-center rounded-[16px] border border-[#e5e7eb] bg-white text-center"
            >
              <div className="h-8 w-8 animate-spin rounded-full border-[3px] border-[#dbeafe] border-t-[#2563eb]" />
              <p className="mt-3 text-[13px] font-bold text-[#4b5563]">
                PC TOP300 순위를 조회하고 있습니다...
              </p>
            </div>
          ) : null}
          {mode === "modal" && error ? (
            <div
              role="alert"
              className="rounded-[12px] border border-red-200 bg-red-50 px-4 py-3 text-[13px] font-semibold text-red-700"
            >
              {error}
            </div>
          ) : null}
          ...
          {displayedAnalysis ? (
            <>
              {mode === "modal" ? (
                <div className="flex flex-wrap items-center justify-between gap-2 rounded-[12px] border border-[#e5e7eb] bg-white px-3 py-2.5 text-[11px] font-semibold text-[#6b7280] md:px-4 md:text-[12px]">
                  <span>
                    {rankHistory?.currentDate ?? "오늘"} 순위 기준
                  </span>
                  <span>총 {formatCount(displayedAnalysis.total)}개</span>
                </div>
              ) : null}
              {mode === "page" ? (
                  ...
                  >
                    {visibleResults.map((row) => (
                      <PlaceRankMobileResultItem
                        ...
                        comparisonActive={selectedSnapshot !== null}
                      />
                    ))}
                  </div>
                </div>
                        ...
                        >
                          ...
                          <td className="px-3 py-2.5">
                            <div className="flex min-w-0 items-center gap-2.5">
                              {row.thumbnail ? (
                                <img
                                  src={row.thumbnail}
                                  alt={`${decodeHtmlText(row.name)} 대표 이미지`}
                                  width={40}
                                  height={40}
                                  loading="lazy"
                          ...
                          <td className="px-3 py-2.5 text-right font-bold tabular-nums text-[#374151]">
                            <PlaceRankMetricValue
                              value={formatTextMetric(row.rating)}
                              currentValue={parseNumericMetric(row.rating)}
                              previousValue={previousMetrics?.get(row.placeId)?.rating}
                              comparisonActive={selectedSnapshot !== null}
                              fractionDigits={2}
                            />
                          </td>
                          <td className="px-3 py-2.5 text-right font-semibold tabular-nums text-[#4b5563]">
                            <PlaceRankMetricValue
                              value={formatCount(row.visitorReviewCount)}
                              currentValue={row.visitorReviewCount}
                              previousValue={
                                previousMetrics?.get(row.placeId)?.visitorReviewCount
                              }
                              comparisonActive={selectedSnapshot !== null}
                            />
                          </td>
                          <td className="px-3 py-2.5 text-right font-semibold tabular-nums text-[#4b5563]">
                            <PlaceRankMetricValue
                              value={formatCount(row.blogReviewCount)}
                              currentValue={row.blogReviewCount}
                              previousValue={
                                previousMetrics?.get(row.placeId)?.blogReviewCount
                              }
                              comparisonActive={selectedSnapshot !== null}
                            />
                          </td>
                          <td className="px-3 py-2.5 text-right font-semibold tabular-nums text-[#4b5563]">
                            <PlaceRankMetricValue
                              value={formatTextMetric(row.saveCount)}
                              currentValue={row.saveCountValue}
                              previousValue={
                                previousMetrics?.get(row.placeId)?.saveCount
                              }
                              currentIsApproximate={row.saveCountIsApproximate}
                              previousIsApproximate={
                                previousMetrics?.get(row.placeId)
                          ...
                          <td className="px-2 py-2.5 text-center">
                            <div className="flex items-center justify-center gap-1">
                              <a
                                ...
                                aria-label={`${decodeHtmlText(row.name)} 모바일 네이버 플레이스${
                              ...
                              <a
                                ...
                                aria-label={`${decodeHtmlText(row.name)} PC 네이버 플레이스${
                                  isPreview ? " (로그인 필요)" : " 새 탭에서 열기"
                                }`}
